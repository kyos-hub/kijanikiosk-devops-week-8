...
In our testing, a deleted service copy was automatically replaced with no manual restart [recovery time: see self-healing-rerun.txt once regenerated]. Separately, packaging the application efficiently — stripping out build tools and unnecessary files — brought the production image down to 48.4MB, comfortably under our 100MB ceiling. [Image-size reduction versus a single-stage baseline is not yet measured; a single-stage build has not been produced for comparison. Once built, replace this bracket with: baseline size, production size, and percentage reduction.] Smaller packages start faster and move through our systems more quickly.
