...
In our testing, a deleted service copy was automatically replaced with no manual restart [recovery time: see self-healing-rerun.txt once regenerated]. Separately, packaging the application efficiently — stripping out build tools and unnecessary files — brought the production image down to 48.4MB, comfortably under our 100MB ceiling. Separately, stripping build tools and unnecessary layers via a multi-stage build cut the image size by 87.8% compared to a naive single-stage build (398MB down to 48.4MB). Smaller packages start faster and move through our systems more quickly.
